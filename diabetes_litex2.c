...
int benchmark(void);
double dot(size_t size, double*x, ...);
...
double dot(size_t size, double *x, ...) {
    ...
    for (uint16_t i = 0; i < 64; i++) {
        const double wi = va_arg(w, double);
        dot += x[i] * wi;
    }
...
int benchmark(void) {
    ...
    printf("\n=== Single Prediction Comparison ===\n");
    double single_fp = predict(input2);
