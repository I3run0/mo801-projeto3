...
typedef double (*dot_function_t)(size_t size, double *x, double *w);
int predict_generic(double *x, dot_function_t dot_func);
void print_statistics(uint32_t *times, int count, const char* name);
...
/**
 * Generic prediction function that works with any dot product implementation
 */
int predict_generic(double *x, dot_function_t dot_func) {
    double votes[NUM_CLASSES];
    ...
    // Compute votes using the provided dot product function
    for (int i = 0; i < NUM_CLASSES; i++) {
        votes[i] += dot_func(FEATURE_SIZE, x, (double*)weights[i]);
    }
...
int predict_hw(double *x) {
    return predict_generic(x, (dot_function_t)logistic_accel_dot_product);
}
...
void print_statistics(uint32_t *times, int count, const char* name) {
    ...
    uint32_t avg_time = (uint32_t)(total_time / count);
...
int benchmark(void) {
    ...
    double input_data[FEATURE_SIZE] = {
    ...
    volatile int cpu_total = 0;
    volatile int hw_total = 0;
    uint32_t cpu_times[BENCHMARK_ITERATIONS];
    uint32_t hw_times[BENCHMARK_ITERATIONS];
    int verification_errors = 0;
    
    // CPU benchmark with individual timing measurements
    printf("Running CPU benchmark (%d iterations)...\n", BENCHMARK_ITERATIONS);
    for (int i = 0; i < BENCHMARK_ITERATIONS; i++) {
        start_stopwatch();
        int result = predict(input_data);
        stop_stopwatch();
        cpu_times[i] = get_elapsed_ticks();
        cpu_total += result;
    }
    
    print_statistics(cpu_times, BENCHMARK_ITERATIONS, "CPU Benchmark");

#if DOT_PRODUCT_ACCEL_AVAILABLE 
    // Hardware benchmark with individual timing measurements
    printf("Running Hardware acceleration benchmark (%d iterations)...\n", BENCHMARK_ITERATIONS);
    for (int i = 0; i < BENCHMARK_ITERATIONS; i++) {
        start_stopwatch();
        int result = predict_hw(input_data);
        stop_stopwatch();
        hw_times[i] = get_elapsed_ticks();
        hw_total += result;
    }
    
    print_statistics(hw_times, BENCHMARK_ITERATIONS, "Hardware Accelerated Benchmark");
    
    // Verify results consistency
    printf("Verifying result consistency...\n");
    for (int test = 0; test < 100; test++) {
        int cpu_result = predict(input_data);
        int hw_result = predict_hw(input_data);
        if (cpu_result != hw_result) {
            verification_errors++;
            if (verification_errors <= 5) {  // Only print first 5 errors
                printf("Verification error #%d: CPU=%d, HW=%d\n", 
                       verification_errors, cpu_result, hw_result);
            }
        }
    }
    ...
    if (verification_errors == 0) {
        ...
        printf("%d verification errors detected!\n", verification_errors);
    }
#endif    
    ...
    // Calculate speedup
    uint64_t cpu_total_time = 0, hw_total_time = 0;
    for (int i = 0; i < BENCHMARK_ITERATIONS; i++) {
        cpu_total_time += cpu_times[i];
        hw_total_time += hw_times[i];
    }
    
    if (hw_total_time > 0) {
        uint32_t speedup_x100 = (uint32_t)((cpu_total_time * 100) / hw_total_time);
        printf("Speedup: %ld.%02ldx\n", speedup_x100 / 100, speedup_x100 % 100);
    }
#endif    
...
static void help(void)
{
    ...
    puts("benchmark                         - Run dot product benchmark");
}
